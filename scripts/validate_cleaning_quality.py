...
if abs(row_diff) <= 120:
    print(f"  ✅ Dentro de tolerancia (±120 filas)")
else:
    print(f"  ❌ Fuera de tolerancia")
    ...
    if result['mismatches'] > 0:
        ...
        if df_joined[ref_col].dtype in [pl.Float64, pl.Float32]:
            mismatch_mask = [
                ...
                for r, c in zip(df_joined[ref_col].to_list(), df_joined[clean_col].to_list())
            ]
        else:
...
numeric_cols = [col for col in base_cols 
                if df_reference[col].dtype in [pl.Float64, pl.Int64]]
...
for col in numeric_cols[:5]:  # Primeras 5 columnas numéricas
    ...
    for stat_name, ref_stat, clean_stat in stats:
        if ref_stat is not None and clean_stat is not None and ref_stat != 0:
            ...
            print(f"{col:<50} {stat_name:<10} {ref_stat:>12.2f} {clean_stat:>12.2f} {diff_pct:>9.2f}% {status}")
