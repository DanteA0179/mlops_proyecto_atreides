...
def sample_db(temp_dir):
    ...
    df = pl.DataFrame({
        ...
        'Load_Type': ['Light', 'Medium', 'Maximum', 'Light', 'Medium', 'Maximum', 'Light', 'Medium'],
        'Day_of_week': ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday', 'Monday'],
        'WeekStatus': ['Weekday', 'Weekday', 'Weekday', 'Weekday', 'Weekday', 'Weekend', 'Weekend', 'Weekday'],
...
class TestQuickQuery:
    ...
    def test_quick_query_with_filter(self, sample_db):
        ...
        df = quick_query(
            "SELECT * FROM steel_cleaned WHERE Load_Type = 'Light'",
            db_path=sample_db
        )
        assert len(df) == 3
    ...
    def test_quick_query_pandas_output(self, sample_db):
        ...
        df = quick_query(
            "SELECT * FROM steel_cleaned LIMIT 5",
            db_path=sample_db,
