...
class TestIdentifyFeatureTypes:
    ...
    def test_basic_identification(self, sample_dataframe):
        ...
        types = identify_feature_types(
            sample_dataframe, exclude_cols=["target"]
        )
        ...
        df = pl.DataFrame(
            {
                ...
                "string_col": ["A", "B", "A", "A", "B", "A", "A", "B", "A", "A"] * 2,  # 2 unique / 20 = 10%
                "bool_col": [True, False, True, True, False, True, True, False, True, True] * 2,
                "datetime_col": pl.date_range(pl.date(2020, 1, 1), pl.date(2020, 1, 20), interval="1d", eager=True),
            }
        )
...
class TestCalculateScalingStatistics:
    ...
    def test_basic_statistics(self, sample_dataframe):
        ...
        stats = calculate_scaling_statistics(
            sample_dataframe, features=["numeric1", "numeric2"]
        )
    ...
    def test_missing_features(self, sample_dataframe):
        ...
        stats = calculate_scaling_statistics(
            sample_dataframe, features=["numeric1", "nonexistent"]
        )
...
class TestAnalyzeCategoricalCardinality:
    ...
    def test_basic_analysis(self, sample_dataframe):
        ...
        analysis = analyze_categorical_cardinality(
            sample_dataframe, feature="categorical"
        )
...
class TestMapBinaryFeature:
    ...
    def test_weekday_weekend_mapping(self):
        ...
        df = pl.DataFrame(
            {"week_status": ["Weekday", "Weekend", "Weekday", "Weekend"]}
        )
    ...
    def test_missing_feature(self):
        ...
        with pytest.raises(ValueError, match="not found in dataframe"):
            map_binary_feature(
                df, feature="nonexistent", mapping={"A": 0, "B": 1}
            )
    ...
    def test_unknown_categories(self):
        ...
        df_mapped = map_binary_feature(
            df, feature="status", mapping={"A": 0, "B": 1}
        )
...
class TestGetFeatureNameAfterOHE:
    ...
    def test_custom_separator(self):
        ...
        name = get_feature_name_after_ohe(
            "Load_Type", "Medium_Load", prefix_sep="-"
        )
