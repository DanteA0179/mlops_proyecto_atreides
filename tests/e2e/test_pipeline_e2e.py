...
class TestDataLoadingPipeline:
    ...
    def test_load_to_duckdb(self, sample_data_path: Path, temp_pipeline_dir: Path):
        ...
        load_csv_to_duckdb(
            csv_path=str(sample_data_path), db_path=str(db_path), table_name="steel_raw"
        )
...
class TestFeatureEngineeringPipeline:
    ...
    def test_cyclical_encoding(self):
        ...
        df = pl.DataFrame({"hour": [0, 6, 12, 18, 23], "day_of_week": [0, 1, 2, 3, 6]})
    ...
    def test_feature_pipeline_integration(self, cleaned_data: pl.DataFrame):
        ...
        pipeline = Pipeline([("temporal", TemporalFeatureEngineer(nsm_column="NSM"))])
...
class TestCompletePipeline:
    ...
    def test_full_pipeline_workflow(self, sample_data_path: Path, temp_pipeline_dir: Path):
