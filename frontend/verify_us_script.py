...
def verify_us032() -> tuple[int, int]:
    ...
    required_modules = ["streamlit", "plotly", "pandas", "requests"]
    ...
    try:
        with open("streamlit_app.py", encoding="utf-8") as f:
            ...
            if (
                "st.tabs" in content
                and '["🏠 Inicio", "🔮 Predicción", "💬 Copiloto IA"]' in content
            ):
    ...
    try:
        with open("pages/1_🔮_Predicción.py", encoding="utf-8") as f:
            content = f.read()
    ...
    try:
        with open("pages/2_💬_Copiloto_IA.py", encoding="utf-8") as f:
            content = f.read()
    ...
    check_service_running("http://localhost:8000/health", "API Backend", optional=True)
    check_service_running("http://localhost:11434/api/tags", "Ollama", optional=True)
...
def print_summary(us032_passed: int, us032_total: int, us033_passed: int, us033_total: int):
    ...
    print(
        f"US-032 (Streamlit UI):       {us032_passed}/{us032_total} ({'✅ PASS' if us032_passed == us032_total else '⚠️  PARTIAL'})"
    )
    print(
        f"US-033 (Deployment):         {us033_passed}/{us033_total} ({'✅ PASS' if us033_passed == us033_total else '⚠️  PARTIAL'})"
    )
    print(
