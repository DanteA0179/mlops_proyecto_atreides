...
def visualize_nulls(null_df: pl.DataFrame, dataset_name: str, color: str):
    ...
    for i, (count, pct) in enumerate(zip(plot_data['null_count'], plot_data['null_percentage'])):
...
def visualize_null_comparison(comparison_df: pl.DataFrame):
    ...
    plt.barh([i - width/2 for i in x], plot_data['clean_null_count'], 
            width, label='Clean', color=COLORS['clean'], alpha=0.7)
    plt.barh([i + width/2 for i in x], plot_data['dirty_null_count'], 
            width, label='Dirty', color=COLORS['dirty'], alpha=0.7)
...
def visualize_outliers_boxplots(
    ...
    plt.suptitle(f'{dataset_name} - Outlier Detection (Boxplots)', 
...
def visualize_type_validation(comparison_df: pl.DataFrame):
    ...
    clean_valid = (comparison_df['clean_valid'] == True).sum()
    clean_invalid = (comparison_df['clean_valid'] == False).sum()
    dirty_valid = (comparison_df['dirty_valid'] == True).sum()
    dirty_invalid = (comparison_df['dirty_valid'] == False).sum()
    ...
    ax1.pie(clean_data, labels=clean_labels, colors=colors_clean, autopct='%1.1f%%',
    ...
    ax2.pie(dirty_data, labels=dirty_labels, colors=colors_dirty, autopct='%1.1f%%',
            startangle=90, textprops={'fontsize': 12})
    ...
    bars1 = ax.bar(x - width/2, clean_counts, width, label='Clean Dataset',
                   color=COLORS['clean'], alpha=0.7)
    bars2 = ax.bar(x + width/2, dirty_counts, width, label='Dirty Dataset',
                   color=COLORS['dirty'], alpha=0.7)
    ...
    ax.set_title('Type Validation Comparison: Clean vs Dirty Dataset',
...
def visualize_duplicate_comparison(comparison_df: pl.DataFrame):
    ...
    plt.bar([i - width/2 for i in x], plot_data['clean_count'], 
           width, label='Clean', color=COLORS['clean'], alpha=0.7)
    plt.bar([i + width/2 for i in x], plot_data['dirty_count'], 
           width, label='Dirty', color=COLORS['dirty'], alpha=0.7)
    
    # Add value labels
    for i, (clean, dirty) in enumerate(zip(plot_data['clean_count'], plot_data['dirty_count'])):
    ...
    plt.title('Duplicate Detection Comparison: Clean vs Dirty', 
             fontsize=14, fontweight='bold')
    plt.legend()
...
def visualize_range_violations(comparison_df: pl.DataFrame):
    ...
    plt.barh([i - width/2 for i in x], plot_data['clean_violations'], 
            width, label='Clean', color=COLORS['clean'], alpha=0.7)
    plt.barh([i + width/2 for i in x], plot_data['dirty_violations'], 
            width, label='Dirty', color=COLORS['dirty'], alpha=0.7)
    
    plt.yticks(x, plot_data['column'])
    plt.xlabel('Violation Count', fontsize=12)
    plt.ylabel('Column', fontsize=12)
    plt.title('Range Violations Comparison: Clean vs Dirty', 
             fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(axis='x', alpha=0.3)
    plt.tight_layout()
