...
def prepare_chronos_finetuning_data_with_covariates(
    ...
    if past_covariates and future_covariates:
        ...
        logger.info(f"✓ Covariates constraint validated")
    ...
    if series_id_col is None or series_id_col not in df.columns:
        ...
        if past_covariates:
            ...
            if past_cov_dict:
                ...
                logger.info(f"Added {len(past_cov_dict)} past covariates: {list(past_cov_dict.keys())}")
        ...
        if future_covariates:
            ...
            if future_cov_dict:
                ...
                logger.info(f"Added {len(future_cov_dict)} future covariates: {list(future_cov_dict.keys())}")
        ...
        for series_id, group in df.group_by(series_id_col):
            input_dict = {
