...
def validate_ranges(df: pl.DataFrame, range_rules: dict, dataset_name: str = "Dataset") -> dict:
    ...
    for column, rules in range_rules.items():
        ...
        try:
            ...
            violation_percentage = (
                (violation_count / non_null_count) * 100 if non_null_count > 0 else 0
            )
    ...
    print(
        f"  Columns with violations: {sum(1 for v in validation_results.values() if v['violations'] > 0)}"
    )
...
def show_range_violation_examples(validation_results: dict, dataset_name: str):
    ...
    for column, results in validation_results.items():
        if results["violations"] > 0:
            ...
            print(f"\n  Column: {column}")
            print(
                f"    Expected range: [{results.get('min_expected', 'N/A')}, {results.get('max_expected', 'N/A')}]"
            )
            print(
                f"    Violations: {results['violations']:,} ({results['violation_percentage']:.2f}%)"
            )
...
def validate_categorical_values(
    df: pl.DataFrame, column: str, valid_values: list, dataset_name: str = "Dataset"
) -> dict:
    """
    ...
    """
    if column not in df.columns:
        return {"status": "column_not_found", "invalid_count": 0, "invalid_values": []}
