"""
XGBoost model training and optimization utilities.

This module provides functions for training XGBoost models with cross-validation
and hyperparameter optimization using Optuna.

Functions:
    create_xgboost_pipeline: Create sklearn pipeline with XGBoost model
    train_xgboost_with_cv: Train XGBoost with cross-validation
    optimize_xgboost_with_optuna: Optimize hyperparameters with Optuna
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            if build_info.get("USE_CUDA", False):
                ...
                logger.info("CUDA version: %s", build_info.get("CUDA_VERSION", "Unknown"))
                return True, gpu_name
            else:
                logger.warning("GPU detected but XGBoost not compiled with CUDA support")
                return False, "CPU (XGBoost without CUDA)"
        else:
...
logger.info("XGBoost configured for: %s", DEVICE_INFO)
logger.info("Using tree_method: %s", DEFAULT_PARAMS["tree_method"])

# Optuna search space (optimized for GPU training speed)
SEARCH_SPACE = {
    "max_depth": (3, 10),
    "min_child_weight": (1, 10),
    "gamma": (0.0, 5.0),
    "learning_rate": (0.01, 0.3),
    "n_estimators": (50, 300),  # Reduced from 1000 to 300 for faster training
    "subsample": (0.5, 1.0),
...
def create_xgboost_pipeline(
    ...
    model_params : dict, optional
        XGBoost hyperparameters (if None, uses DEFAULT_PARAMS)
    ...
    >>> pipeline = create_xgboost_pipeline(model_params=params)
...
def train_xgboost_with_cv(
    ...
    >>> results = train_xgboost_with_cv(X_train, y_train, params, cv_folds=5)
    ...
    try:
        ...
        # Create pipeline
        pipeline = create_xgboost_pipeline(model_params=model_params)
...
def optimize_xgboost_with_optuna(
    ...
    try:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            # MLflow nested runs disabled to avoid Windows encoding issues
            # Trial results saved in trials_df instead
        ...
        best_pipeline = create_xgboost_pipeline(model_params=best_params)
