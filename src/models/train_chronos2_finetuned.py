...
This script fine-tunes Chronos-2 on the steel energy consumption dataset
and evaluates the fine-tuned model against zero-shot and XGBoost baselines.
"""
...
import mlflow
from src.models.chronos2_finetuning import (
    finetune_chronos2,
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Chronos-2 model")
    parser.add_argument(
        "--context-length",
        type=int,
        default=512,
    ...
    parser.add_argument(
        "--experiment-name",
        type=str,
        default="steel_energy_chronos2_finetuned",
    ...
    # Fine-tuning
    logger.info("\n[4/6] Fine-tuning Chronos-2")

    with mlflow.start_run(run_name=f"chronos2_finetuned_{timestamp}"):
        # Log parameters
        mlflow.log_param("model_name", model_name)
        mlflow.log_param("model_type", "chronos2_finetuned")
        ...
        # Fine-tune WITHOUT covariates (simple baseline)
        logger.info("Fine-tuning without covariates (baseline)")
        ...
        save_path = output_dir / f"chronos2_finetuned_{timestamp}"
        ...
        # Save results
        results_file = output_dir / f"chronos2_finetuned_results_{timestamp}.json"
        results_data = {
            "model": model_name,
            "finetuned": True,
            "num_steps": args.num_steps,
