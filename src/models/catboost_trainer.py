"""
CatBoost model training and optimization utilities.

This module provides functions for training CatBoost models with cross-validation
and hyperparameter optimization using Optuna. CatBoost handles categorical features
natively, which is a key advantage.

Functions:
    check_gpu_availability: Check if GPU is available for CatBoost
    create_catboost_pipeline: Create sklearn pipeline with CatBoost model
    train_catboost_with_cv: Train CatBoost with cross-validation
    optimize_catboost_with_optuna: Optimize hyperparameters with Optuna
...
def check_gpu_availability() -> tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            logger.info("CatBoost will attempt to use GPU with task_type='GPU'")
...
# Default CatBoost parameters (GPU-aware)
DEFAULT_PARAMS = {
    "loss_function": "RMSE",
...
logger.info("CatBoost configured for: %s", DEVICE_INFO)
logger.info("Using task_type: %s", DEFAULT_PARAMS["task_type"])
...
def create_catboost_pipeline(
    ...
    """
    Create sklearn pipeline with CatBoost model.
    ...
    model_params : dict, optional
        CatBoost hyperparameters (if None, uses DEFAULT_PARAMS)
    cat_features : list of str or list of int, optional
    ...
    -------
    Pipeline
        sklearn pipeline with CatBoost regressor

    Examples
    --------
    >>> params = {'depth': 6, 'learning_rate': 0.1}
    >>> cat_features = ['Load_Type', 'WeekStatus']
    ...
    try:
        ...
        if "task_type" not in model_params and GPU_AVAILABLE:
        ...
        if use_preprocessing:
            ...
            pipeline = Pipeline([("model", catboost_model)])
...
def train_catboost_with_cv(
    ...
    >>> cat_features = ['Load_Type', 'WeekStatus']
    >>> results = train_catboost_with_cv(
    ...
    try:
        ...
        # Create pipeline
        pipeline = create_catboost_pipeline(model_params=model_params, cat_features=cat_features)
...
def optimize_catboost_with_optuna(
    ...
    --------
    >>> cat_features = ['Load_Type', 'WeekStatus']
    >>> results = optimize_catboost_with_optuna(
    ...
    try:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            params = {
                ...
                "l2_leaf_reg": trial.suggest_int("l2_leaf_reg", *SEARCH_SPACE["l2_leaf_reg"]),
            ...
            pipeline = create_catboost_pipeline(model_params=params, cat_features=cat_features)
            pipeline.fit(X_train, y_train)
        ...
        best_pipeline = create_catboost_pipeline(
            model_params=best_params, cat_features=cat_features
        )
